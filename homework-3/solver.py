...
def train(model, criterion, optimizer, dataset, max_epoch, batch_size,
          disp_freq):
    ...
    with tf.Session(config=config) as sess:
        ...
        train_data = tf.data.Dataset.zip(
            (tmp1, tmp2)).repeat(max_epoch).batch(batch_size)
        ...
        valid_data = tf.data.Dataset.zip(
            (tmp3, tmp4)).repeat(max_epoch).batch(batch_size)
        ...
        for epoch in range(max_epoch):
            batch_train_loss, batch_train_acc = train_one_epoch(
                model, criterion, optimizer, train_get_next, max_epoch,
                batch_size, disp_freq, epoch, sess)
            batch_val_loss, batch_val_acc = validate(model, criterion,
                                                     valid_get_next,
                                                     batch_size, sess)
            ...
            print()
            print(
                'Epoch [{}]\t Average training loss {:.4f}\t Average training accuracy {:.4f}'
                .format(epoch, avg_train_loss[-1], avg_train_acc[-1]))

            print(
                'Epoch [{}]\t Average validation loss {:.4f}\t Average validation accuracy {:.4f}'
                .format(epoch, avg_val_loss[-1], avg_val_acc[-1]))
            print()

    return model, avg_val_loss, avg_val_acc


def train_one_epoch(model, criterion, optimizer, data_get_next, max_epoch,
                    batch_size, disp_freq, epoch, sess):
    ...
    for iteration in range(max_train_iteration):
        ...
        if iteration % disp_freq == 0:
            print(
                ...
                .format(epoch, max_epoch, iteration, max_train_iteration,
